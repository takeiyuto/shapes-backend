...
contract.events.MintRequested(async (error, result) => {
    ...
    const tokenId = <number>web3.utils.toNumber(result.returnValues.tokenId);
...
async function processRequest(tokenId: number) {
    ...
    const timestamp = <number>web3.utils.toNumber(request.timestamp);
    const expectedCid = await addMetadata(tokenId, timestamp);
...
async function sendTransaction(tx: NonPayableTransactionObject<void>) {
    ...
    await tx.send({ from: myAccount.address, gas: estimatedGas * 1.5 });
}
...
for (let tokenId = 1; tokenId <= 1024; tokenId++) {
    if (await contract.methods.requested(tokenId).call()) {
        ...
        const timestamp = <number>web3.utils.toNumber(timestampText);
        const cid = await addMetadata(tokenId, timestamp);
